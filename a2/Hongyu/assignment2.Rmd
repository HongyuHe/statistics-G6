...
(2) Yes, a test result that is positive will increase the risk of having cancer because
```
...
x~57~ = (8, 1); x~58~ = (8, 2); x~59~ = (8, 3); x~60~ = (8, 4); x~61~ = (8, 5); x~62~ = (8, 6); x~63~ = (8, 7); x~64~ = (8, 8); 
  ...
  2) Their probabilities: P(X=x~1~) == P(X=x~2~) == ... == P(X=x~64~) == 1/64
...
### d) *Proof*: Var(Y) = 0^2^ * 49/64 + 1^2^ * 7/32 + 2^2^ * 1/64 - E(Y)^2^ = 7/32 

### e)

Since the sample size is much greater than 30, according to the **central limit theorem**, the random variables **X~i~** approximately have a normal distribution with an expectation **$\mu$** of 1/4 and a standard deviation of **$\frac{\sigma}{\sqrt{n}}$**, which is $$N(1/4, \frac{7/32}{n})$$
